...
class DragPage extends StatefulWidget {
  static const String routeName = '/page/page_drag';
  static const String name = "DragPage";
...
class _DragPageState extends State<DragPage> {
  ...
  Widget _buildAppbar() {
    ...
    statusBarHeight = MediaQuery.of(context).padding.top;/*状态栏高度*/
...
class _DragDestinationState extends State<DragDestination> {
  ...
  _startTimer() {
    /*创建循环*/
    _timer = new Timer.periodic(new Duration(seconds: 4), (timer) {
      setState(() {
        _randomColor();
      });
    });
  ...
  Widget build(BuildContext context) {
    return Positioned(
      right: 0.0,
      child: DragTarget(
        /*DragModel Draggable 携带过来的数据*/
        onAccept: (DragModel model) {
          showShortToast("==="+model.itemName);
        ...
        builder: (
            ...
            List<dynamic> accepted,/*一次性拖拽过来的个数*/
          ...
          return Container(
            ...
            child: Center(
              child: Text(
                "放入正确的颜色\n您的得分：$_score=="+accepted.length.toString()+"==="+rejected.length.toString(),
    ...
    return Positioned(
        ...
        child: Draggable(
          ...
          feedback: Container(
            width: drag_item_height,
            height: drag_item_height,
            color: widget.dragModel.itemBgColor.withOpacity(0.1),
            child: Center(
              child: Text(
                widget.dragModel.itemName,
                style: TextStyle(
